...
const Combineddashboard = () => {
  ...
  const propertyData = [
    ...
    },
    {
      ...
      bathrooms: 2,
      type: "sale",
      image: "https://images.unsplash.com/photo-1600585154340-be6161a56a0c?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1500&q=80",
    },
    {
  ...
  const { totalProperties, forSale, forRent, onOffer } = useMemo(() => {
    const total = listings.length;
    const sale = listings.filter((l) => l.type === "sale").length;
    const rent = listings.filter((l) => l.type === "rent").length;
    const offer = listings.filter((l) => l.offer === true).length;
    return { totalProperties: total, forSale: sale, forRent: rent, onOffer: offer };
  }, [listings]);
  ...
  ], [totalProperties, forSale, forRent, onOffer]);

  // Graph data
  const graphData = useMemo(() => ({
    total: listings.map((l, i) => ({ name: `Prop ${i + 1}`, value: 1 })),
    sale: listings.map((l, i) => ({
      name: `Prop ${i + 1}`,
      value: l.type === "sale" ? 1 : 0,
    })),
    rent: listings.map((l, i) => ({
      name: `Prop ${i + 1}`,
      value: l.type === "rent" ? 1 : 0,
    })),
    offer: listings.map((l, i) => ({
      name: `Prop ${i + 1}`,
      value: l.offer ? 1 : 0,
    })),
  }), [listings]);
  ...
  const pieDataType = useMemo(() => [
    { name: "For Sale", value: forSale, color: '#2ECC71' },
    { name: "For Rent", value: forRent, color: '#3498DB' },
  ], [forSale, forRent]);

  const pieDataOffer = useMemo(() => [
    { name: "On Offer", value: onOffer, color: '#E67E22' },
    { name: "Regular", value: totalProperties - onOffer, color: '#BDC3C7' },
  ], [onOffer, totalProperties]);

  // --- Dynamic Graph Props ---
  const RENT_BAR_COLORS = [
      '#3498DB', '#5DADE2', '#85C1E9', '#AED6F1', '#D6EAF8', '#2E86C1', '#2874A6',
  ];

  const currentBarColors = selectedTile === 'rent' ? RENT_BAR_COLORS : undefined;

  const graphTitle = selectedTile === 'rent'
    ? "Properties For Rent"
    : tiles.find((t) => t.id === selectedTile)?.title || "Property Distribution";
  // ---------------------------
    ...
    <div className="p-4 sm:p-6 min-h-screen pt-16 bg-gray-100 mt-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {tiles.map((tile) => (
          <div
            ...
            className={`cursor-pointer transform hover:scale-[1.03] transition-transform duration-200 ${selectedTile === tile.id ? 'ring-4 ring-offset-2 ring-[#1F4B43]/50 rounded-xl' : ''}`}
          >
            <Tile
              title={tile.title}
              value={tile.value}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-10">
       ...
       <div className="bg-white p-8 rounded-3xl shadow-2xl w-full h-full">
        <h2 className="text-2xl font-extrabold mb-6 text-[#1F4B43] tracking-tight text-center">View Recently Added Properties</h2>
        {tabsData.map(tab => (
        <button
            key={tab}
      ...
      <div className="mt-10">
        <h2 className="text-2xl font-semibold mb-6 text-[#1F4B43] border-b pb-2">Featured Property Listings</h2>
