...
const ListingDetails = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // 2. Create booking → backend will emit real-time notification
      ...
      toast.success(
        isSellerOnline
          ? "Message sent! Seller is online — instant notification sent."
          : "Message sent! Seller will be notified when they come online."
      );
          ...
          <div className="relative h-[60vh] overflow-hidden group">
            {listing.images?.length > 0 ? (
              <>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-8">
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    ...
                    <span className={`px-6 py-2 rounded-full text-sm font-bold text-white backdrop-blur-md ${listing.type === "rent" ? "bg-blue-500/80" : "bg-green-500/80"} shadow-lg`}>
                ...
                {listing.images.length > 1 && (
                  <>
                    <button onClick={handlePrevImage} className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/20 backdrop-blur-md text-white rounded-full hover:bg-white/30 transition-all duration-300 flex items-center justify-center">
                      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                      </svg>
                    </button>
                    <button onClick={handleNextImage} className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/20 backdrop-blur-md text-white rounded-full hover:bg-white/30 transition-all duration-300 flex items-center justify-center">
                      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-purple-900/50 to-pink-900/50">
                <span className="text-white/50 text-xl font-medium">No Image Available</span>
              </div>
            )}
          </div>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gradient-to-br from-purple-500/20 to-pink-500/20 backdrop-blur-xl rounded-2xl p-4 border border-white/10">
                  <p className="text-xs font-semibold text-purple-300 uppercase tracking-wider mb-1">Price</p>
                  <div className="text-xl font-bold text-white">
                    ${listing.offer ? listing.discountedPrice.toLocaleString() : listing.regularPrice.toLocaleString()}
                    {listing.type === "rent" && <span className="text-lg text-purple-300"> /month</span>}
                  </div>
                  {listing.offer && (
                    <div className="mt-2 flex items-center gap-5">
                      <span className="text-white/50 line-through text-lg">${listing.regularPrice.toLocaleString()}</span>
                ...
                {userId && userId !== listing.userId && (
                  <>
                    <div className="flex items-center gap-3 p-4 bg-white/5 rounded-xl border border-white/10">
                      <div className="relative">
                        <div className={`w-4 h-4 rounded-full ${isSellerOnline ? "bg-green-400" : "bg-gray-600"} shadow-lg`}></div>
                        {isSellerOnline && <div className="absolute inset-0 w-4 h-4 bg-green-400 rounded-full animate-ping"></div>}
                      </div>
                      <div>
                        <p className={`font-semibold text-lg ${isSellerOnline ? "text-green-300" : "text-gray-400"}`}>
                          {isSellerOnline ? "Seller is Online" : "Seller is Offline"}
                        </p>
                        <p className="text-xs text-gray-400">
                          {isSellerOnline ? "Instant reply possible" : "Will be notified when online"}
                        </p>
                      </div>
                    </div>
